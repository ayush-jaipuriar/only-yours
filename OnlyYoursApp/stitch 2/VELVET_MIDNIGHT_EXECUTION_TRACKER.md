...
- [x] Keep Home focused on current-state hero actions instead of duplicating lower-tab content
- [x] Remove redundant dashboard entry-point cards/buttons
- [x] Move progression / celebrations / stats / achievements ownership out of Home into `Stats`
...
### Notes / Blockers
- Notes: `DashboardScreen` now uses a state-driven Velvet Midnight structure instead of a mostly linear utility stack. The screen keeps the existing `useDashboardGameFlow` contract and navigation behavior, but Home is now intentionally lean: editorial greeting plus a single stateful hero (`active game`, `ready to start`, `results ready`, `not linked`) with the clearest next action. The earlier progression, celebration, stats, and achievements sections were deliberately moved out of Home once the bottom nav expanded to five tabs (`Home`, `History`, `Custom`, `Stats`, `Profile`). This reduces duplication and gives each browse tab a clearer responsibility: Home answers “what should I do now,” Stats answers “how are we doing,” and Profile answers “who am I / what account actions can I take.” Automated validation now covers both halves of the dashboard contract: `DashboardScreen.test.js` verifies the main hero states plus their primary CTAs, and `useDashboardGameFlow.test.js` continues to cover the underlying data/loading behavior. This runtime pass revalidated the `not linked` to `linked + no active game` transition using disposable real accounts on phone and tablet: after linking the pair through the real backend, both devices cold-launched back into the `Ready to Begin` dashboard hero, and `Start New Game` routed correctly into `CategorySelection` on tablet. We then advanced the same real session into `INVITED` and Round 1. After force-stopping the phone mid-session, the app cold-launched back into the correct `ACTIVE SESSION` dashboard hero (`ROUND1 • QUESTION 1/8`), and `Continue Game` routed back into the same live question screen. A later recovery pass extended the hero model with a persisted `results_ready` state backed by `latest_completed_session_v1`, so a just-finished session can now be surfaced from dashboard instead of disappearing on relaunch. Phone runtime verification using a seeded completed-session snapshot confirmed the new `Results ready` hero renders correctly, and the CTA row was moved above the metadata block so the action appears earlier on smaller screens.
...
- [x] Introduce dedicated [`StatsScreen.js`](/Users/ayushjaipuriar/Documents/GitHub/only-yours/OnlyYoursExpo/src/screens/StatsScreen.js) for progression / celebrations / stats / achievements
- [x] Remove unsupported profile concepts such as journal archives
- [ ] Preserve real profile actions:
  - edit profile
  - settings
  - sign out
- [x] Move profile identity ownership to the new `Profile` tab:
  - avatar
  - name
  - email
  - username
  - bio
- [x] Move growth/analytics ownership to the new `Stats` tab:
  - progression
  - recent celebrations
  - achievements
  - game stats
...
- [x] Edit profile still works
- [x] Stats data still renders correctly
...
### Notes / Blockers
- Notes: The browse IA has now been tightened further: the bottom nav is `Home`, `History`, `Custom`, `Stats`, `Profile`. `StatsScreen` owns progression, recent celebrations, achievements, and gameplay metrics, while `ProfileScreen` is now intentionally lean and account-focused: avatar/identity, `Edit Profile`, `Settings`, and `Sign Out`. This split reduces the earlier redundancy where Home and Profile both repeated growth content. Focused Jest coverage now mirrors that split: `ProfileScreenFlow.test.js` protects identity + edit/save behavior, while `StatsScreenFlow.test.js` protects progression/celebration/achievement rendering and share CTA visibility. The first phone runtime pass exposed a malformed `Stats` surface despite green tests, which taught us an important implementation lesson: browse destinations that are likely to hydrate mixed progression/stats data behave more reliably with flatter direct-Velvet composition than with more nested display abstractions. After a clean Metro refresh and a simplified `StatsScreen`, phone runtime now visibly renders progression, celebrations, metrics, and achievements correctly. A follow-up tablet browse sweep now also confirms the new IA behaves coherently on large screens: `Stats` renders its progression-led surface correctly, `Profile` shows the lean account actions, and `Settings` still opens with the themed header plus the expected `Theme`, `Haptics`, and `Notification Preferences` groupings. `SettingsScreen` upper and lower sections also rendered coherently in earlier hardware review, including theme controls, haptics, notification preferences, relationship controls, and replay onboarding. The unlink safeguard is now improved in code for the active-game state: `SettingsScreen` loads `/game/active`, renders unlink as an explicitly unavailable guarded action when a live session exists, and shows pre-tap guidance instead of relying on a post-tap backend rejection. This guarded unlink state is now also visually confirmed on hardware. A later polish pass also tightened the dense explanatory copy in `Settings`: mode, haptics, reminders, unlink, cooldown, and onboarding sections now read more cleanly without changing any contracts. A tablet runtime pass confirms the real sign-out path works from the live profile screen and resets the app back to the Velvet Midnight auth surface. A follow-up tablet re-entry pass with a known-good paired account also successfully restored the live app shell, which confirms the earlier `alphatwo` confusion was stale test-data drift rather than a frontend auth regression. Recover-previous-partner is now hardware-validated on phone end to end: the linked state rendered correctly, unlink opened a final confirmation block with 24-hour cooldown language, confirm unlink transitioned the screen into `COOLDOWN_ACTIVE` with an absolute recovery timestamp and `Recover Previous Partner` CTA, and recovery returned the same account to the linked state. Backend verification after the runtime pass confirms both users in the disposable pair are again `LINKED` to the same couple record.
